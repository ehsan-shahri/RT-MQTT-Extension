...
.. tip::
   ...
   This tutorial contains functions and definitions to use the Real-Time MQTT (RT-MQTT) extention.
...
################################################
The presented system proposes a set of extensions to the MQTT protocol grounded on Software Defined 
Networking (SDN) that enable, at the network level, attaining real-time communication services.
This system conveyes real-time requirements such as deadline, transmition bandwidth and flow priority
to topics and end-nodes by standard MQTT messages via the user properties field and then they are used 
to improve the timeliness of the execution of software components at the broker and to manage the 
network using real-time network manager (RT-NM), creating deterministic communication channels matching 
the real-time requirements of the associated topics.


Components
##########
The proposed RT-MQTT architecture is shown in Fig. 1. The system consists of the 
MQTT application (broker and clients), RT-NM, OF-Controller and OF-Switches components.   
...
* :gray:`Dead-line (Second):`                      It used to enables forwarding and scheduling decisions for time critical MQTT applications.
...
* :gray:`d:`                                  String to define a value as a deadline packet time.
* :gray:`mb:`                                 String to define a value as a minumum data rate bandwidth.
* :gray:`Mb:`                                 String to define a value as a maximum data rate bandwidth.
* :gray:`p:`                                  String to define a value as a flwo priority.
...
ii.Defining call-back function that will runned whenever there is a message (payload) published on the given topic. This function extracts details from the msg user properties: ::
...
OF-Switch
---------
OF-Controller updates the OF-Switches flow tables based on time-sensitive parameters and configurs them to create  
...
Parameter
^^^^^^^^^
* :gray:`bridge_name:`                  Bridge name id.
